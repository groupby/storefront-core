...
import Phase from '../core/tag/phase';
...
class LoggingService extends BaseService<LoggingService.Options> {
  ...
  logObserverNodeChange = ({ path, event, value }: { path: string; event: string; value: any }) =>
    this.app.log.debug(
  ...
  logTagLifecycle = ({ phase, tag }: { phase: string; tag: Tag }) =>
    this.app.log.debug(
  ...
  logTrackerEvent = ({ type, event }: { type: string; event: object }) =>
    this.app.log.debug(`[tracker]: %c${type}`, 'color: #75babe; font-weight: bold', event);

  // tslint:disable-next-line max-line-length
  logAliasing = ({ action, name, type, aliases }: { action: string; name: string; type: string; aliases: any }) =>
    this.app.log.debug(
